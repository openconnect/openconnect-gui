...
int validate_peer_cert(void *privdata, const char *reason)
{
    ...
    if (der_size <= 0) {
	vpn->m->
	    updateProgressBar(QObject::
			      tr("Peer's certificate has invalid size!"));
	return -1;
    }
	...
	vpn->m->
	    updateProgressBar(QObject::
			      tr("Error getting peer's certificate hash"));
	return -1;
    }
	...
	MyCertMsgBox msgBox(vpn->m,
			    ...
			    str,
			    QObject::
			    tr("The key was changed by the administrator"),
			    dstr);
	msgBox.show();
	ok = msgBox.result();
